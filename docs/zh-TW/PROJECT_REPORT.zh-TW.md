...
|---|---|---|---|---|
| Deterministic detection and triage policy | Rule-based classification, deterministic Risk Level / Decision, HIGH -> BLOCK, MEDIUM -> MONITOR, LOW -> ALLOW style behavior. | Rule-Based Detector remains detection authority. | `tests/test_detector.py`, `tests/test_detection_rules.py`, `tests/test_fast_analysis.py`, `tests/test_http2_resource_exhaustion_demo.py` | Does not prove production IDS/IPS effectiveness. |
| Evidence bundle and Evidence-Grounded AI Brief | Official verdict, rule IDs, evidence IDs, citation IDs, evidence gaps, unsafe assumptions. | Official Risk / Decision are copied from deterministic context and not regenerated by AI. | `modules/ai_advisory/evidence_bundle.py`, `modules/ai_advisory/grounded_brief.py`, `tests/test_ai_advisory_evidence_bundle.py`, `tests/test_ai_advisory_grounded_brief.py` | Does not prove every real incident narrative is complete. |
| AI guardrails and safety boundary | Blocks/fallbacks for verdict tampering, Similar Cases-as-proof, Graph-as-detection-source, enforcement wording, exploit / PoC / traffic generation / load testing. | AI/RAG/Similar Cases/Graph remain advisory-only; no real enforcement. | `tests/test_ai_advisory_grounded_brief.py`, `tests/test_llm_guardrails.py`, `tests/test_answer_guardrails.py` | Pattern checks are not a complete production policy engine. |
| v3.1 Full AI-assisted foundation | Prompt contract, provider modes, disabled default, fake test injection, optional local/openai-compatible provider, invalid JSON, missing citations, provider failures, provider exceptions. | CI requires no live LLM/API key/Ollama/Chroma/embeddings/network; official verdict cannot be overwritten. | `modules/ai_advisory/prompt_contract.py`, `modules/ai_advisory/llm_provider.py`, `modules/ai_advisory/full_ai_assisted.py`, `tests/test_ai_advisory_full_ai_assisted.py` | Does not prove live provider quality or availability. |
| Event-aware Q&A backend | Uses deterministic verdict, rule IDs, evidence IDs, evidence gaps, optional RAG, Similar Cases, Graph context; refuses unsafe questions before provider calls. | Similar Cases cannot prove compromise; Graph is not a detection source; no real enforcement recommendation. | `modules/ai_advisory/event_qa.py`, `tests/test_ai_advisory_event_qa.py` | Does not provide a full conversational UI. |
| Similar Cases and Relationship Graph | Approved Similar Cases are deterministic advisory comparisons; Graph is read-only explanatory context. | Historical cases are not proof; Graph is not detection source. | `tests/test_approved_case_retrieval.py`, `tests/test_graph_builder.py`, `tests/test_ui_relationship_graph_view.py` | Does not prove production-scale case memory or graph analytics. |
| RAG / Knowledge Q&A controls | Controlled retrieval, source assembly, answer safety framing, no-answer paths, lazy startup. | RAG remains optional/advisory and does not modify official verdict. | `tests/test_rag_qa_controlled_runtime.py`, `tests/test_rag_intent.py`, `tests/test_rag_controlled_retrieval.py`, `tests/test_lazy_rag_startup.py` | Does not prove live embedding quality or complete knowledge coverage. |
| UI helper / Streamlit smoke behavior | UI helper paths, panel rendering, export view, route/policy view, Run -> Find Similar Cases -> case-001 / graph-001. | UI preserves deterministic/advisory split and avoids stale context regressions. | `tests/test_ui_report_sections.py`, `tests/test_ui_ai_advisory_view.py`, `tests/test_ui_report_export_view.py`, `tests/test_ui_demo_scenarios.py` | Does not prove production browser/deployment readiness. |
| Documentation / release-gate consistency | Docs wording, screenshot references, release-gate notes, safety-boundary language. | Public review material repeats the same authority boundary and limitations. | `docs/TEST_REPORT.md`, `docs/v2.9_release_gate.md`, `docs/screenshots/README.md` | Does not replace human review. |
